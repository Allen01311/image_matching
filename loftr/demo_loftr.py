...
if __name__ == '__main__':
    ...
    with open('outdoor_ds.ckpt', 'rb') as f:
        model_bytes = BytesIO(f.read())

    matcher = LoFTR(config=default_cfg)
    model_dict = torch.load(model_bytes, map_location=torch.device('cpu'))
    matcher.load_state_dict(model_dict['state_dict'])
    ...
    while True:
        ...
        if not opt.no_display:
            ...
            #---------------------------------------------------------
            #print出信心度
            print(f'Confidence values for the current frame:{mconf}')
            
            #print出每張圖平均信心度
            average_confidence = np.mean(mconf)
            print(f'Average Confidence for the current frame: {average_confidence}')

            #所有圖平均信心度
            all_average_confidences.append(average_confidence)
            
            #畫每條特徵匹配的信心程度直方圖
            output_directory = 'confidence_histograms'
            os.makedirs(output_directory, exist_ok=True)
            
            #Plot a histogram of confidence scores
            plt.figure(figsize=(8, 5))
            plt.hist(mconf, bins=20, range=(0, 1), alpha=0.7, color='red')
            ...
            filename = f"{stem}_confidence_histogram.png"
